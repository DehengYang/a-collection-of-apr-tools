...
def main():
    ...
    df_new = pd.DataFrame(columns=['Title', 'APR Tool Name', 'Authors', 'Year', 'Venue', 'Repo URL', 'Target Language', 'Used Dataset', 'CCF Rank', 'Paper Category', 'Bibtex', 'Specification', 'Tool Category', 'Bug Types'])
    for json_file in json_files:
        print(f'Parsing {json_file}...')
        # df = pd.read_json(json_file)
        with open(json_file, 'r', encoding='utf-8') as f:
            data_dict = json.load(f)        
            df_new.loc[len(df_new)] = [data_dict['Title'], data_dict['APR Tool Name'], data_dict['Authors'], data_dict['Year'], data_dict['Venue'], data_dict['Repo URL'], data_dict['Target Language'], data_dict['Used Dataset'], data_dict['CCF Rank'], data_dict['Paper Category'], data_dict['Bibtex'], data_dict['Specification'], data_dict['Tool Category'], data_dict['Bug Types']]
    print(df_new)
