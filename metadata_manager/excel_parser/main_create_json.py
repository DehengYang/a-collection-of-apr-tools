...
from pyutils import Dir_util, File_util
...
def main_json_creator_from_terminal():
    """
    Create a json file in an interactive fashion.
    1. define items;
    2. get the input of each item from user;
    3. save these inputs into a json instance;
    4. output to a json file
    """
    json_output_dir = os.path.join(cur_dir, "apr_literature_metadata")
...
def main_json_creator_from_excel_file(excel_path):
    ...
    df = pd.read_excel(excel_path, sheet_name='工作表1', header=0)  # 表头在第0行（即第一行，Python中索引从0开始）
    # new_excel_path = os.path.join(config.OUTPUT_DIR, 'apr_literature_metadata_new.xlsx')
    df_new = pd.DataFrame(columns=['Title', 'APR Tool Name', 'Authors', 'Year', 'Venue', 'Repo URL', 'Target Language', 'Used Dataset', 'CCF Rank', 'Paper Category', 'Bibtex'])
    for index, row in df.iterrows():
        ...
        target_language = [language.strip() for language in row_dict['Target Language'].split(',')]
        used_dataset = [dataset.strip() for dataset in row_dict['Used Dataset'].split(',')]
        ccf_rank = row_dict['CCF Rank']
        paper_category = '-'
        
        # 3) get and parse bibtex from dblp
        bib_output_file_name = f"{year}{get_formatted_title(title)}.bib"
        if bib_output_file_name in os.listdir(bib_output_dir):
            print(f"[Skip] {bib_output_file_name} already exists.")
            bibtex = File_util.read_file(os.path.join(bib_output_dir, bib_output_file_name))
        else:
            bibtex = bibtex_parser.get_bibtex_from_dblp(title)
            time.sleep(4) # avoid frequent queries to dblp sites. refer to: https://dblp.org/robots.txt
            if bibtex != "":
        ...
        df_new.loc[len(df_new)] = [title, apr_tool_name, authors, year, venue, repo_url, target_language, used_dataset, ccf_rank, paper_category, bibtex]
    ...
    for index, row in df_new.iterrows():
        ...
        row_dict['Bug Types'] = '-'
        json_output_path = os.path.join(json_output_dir, f"{year}-{formmated_title}.json")
...
if __name__ == "__main__":
    main_json_creator_from_excel_file(excel_path=os.path.join(config.PROJ_DIR, 'input/A_collection_of_apr_tools.xlsx'))
